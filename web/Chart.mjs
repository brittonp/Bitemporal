...
export class Chart {
  constructor(canvas, dataManager, opts = {}) {
    ...
    this.hoveredId = null;
    this.baseFontSize =
      parseFloat(getComputedStyle(document.documentElement).fontSize.trim()) ||
      14;
  ...
  #startOfNextYear(date) {
    return new Date(date.getFullYear() + 2, 0, 1);
  }

  #xScale(date) {
    return (
      this.margin.left +
      ((date - this.validMin) / (this.validMax - this.validMin)) *
        this.chartWidth
    );
  }
  ...
  #yScale(date) {
    return (
      this.margin.top +
      this.chartHeight -
      ((date - this.tranMin) / (this.tranMax - this.tranMin)) * this.chartHeight
    );
  }
  ...
  #drawRectangles() {
    ...
    this.data.forEach((d, i) => {
      ...
      const x2 = this.#xScale(
        d.valid_to ? new Date(d.valid_to) : this.validMax
      );
  ...
  #onMouseMove(e) {
    ...
    const chartX = Math.min(
      Math.max(mouseX, this.margin.left),
      this.width - this.margin.right
    );
    const chartY = Math.min(
      Math.max(mouseY, this.margin.top),
      this.height - this.margin.bottom
    );

    const normX = (chartX - this.margin.left) / this.chartWidth;
    const normY =
      (this.chartHeight - (chartY - this.margin.top)) / this.chartHeight;

    const validDate = new Date(
      this.validMin.getTime() + normX * (this.validMax - this.validMin)
    );
    const tranDate = new Date(
      this.tranMin.getTime() + normY * (this.tranMax - this.tranMin)
    );
  ...
  #drawCrosshairs(chartX, chartY, validDate, tranDate, e) {
    ...
    this.data.forEach((d, i) => {
      ...
      if (chartX >= x1 && chartX <= x2 && chartY >= y2 && chartY <= y1) {
        this.tooltip.innerHTML += `<pre><strong>${JSON.stringify(
          d,
          null,
          2
        )}</strong></pre>`;
    ...
    if (this.hoveredId !== hoveredId) {
      ...
      this.dataManager.dispatchEvent(
        new CustomEvent('chart:hover', { detail: { dates } })
      );
    }
  }
  ...
  #onMouseClick(e) {
    ...
    this.dataManager.dispatchEvent(
      new CustomEvent('chart:click', { detail: { dates } })
    );
  }
}
