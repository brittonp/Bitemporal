...
export class DataManager extends EventTarget {
  ...
  async loadEmployees() {
    const data = await this.fetchData(
      `/Bitemporal/GetEmployeeBitemporalData?deptId=${this.config.deptId}&empId=${this.config.empId}`
    );
  ...
  async loadQuery(dates) {
    const data = await this.fetchData(
      `/Bitemporal/GetData?validDate=${encodeURIComponent(
        dates.validDate
      )}&tranDate=${encodeURIComponent(dates.tranDate)}`
    );
    this.#getData = data;
  ...
  async updateData(cmdName) {
    try {
      const resetResponse = await this.fetchData(
        `/Bitemporal/UpdateData/${cmdName}`
      );
