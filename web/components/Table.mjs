...
export class Table {
  constructor(container, dataManager, opts = {}) {
    ...
    if (opts.initialMessage) {
      ...
      msg.className = 'initial-message';
      msg.textContent = opts.initialMessage;
  ...
  highlightRow(dates) {
    ...
    const row = Array.from(this.table.rows).find((r) => {
      ...
      const validFromIdx = headers.findIndex(
        (h) => h.textContent.trim() === 'valid_from'
      );
      const validToIdx = headers.findIndex(
        (h) => h.textContent.trim() === 'valid_to'
      );
      const tranFromIdx = headers.findIndex(
        (h) => h.textContent.trim() === 'tran_from'
      );
      const tranToIdx = headers.findIndex(
        (h) => h.textContent.trim() === 'tran_to'
      );
      ...
      return (
        validDate >= validFromDate &&
        validDate < validToDate &&
        tranDate >= tranFromDate &&
        tranDate < tranToDate
      );
    });
